    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        v_start = rg.Point(intersection_center.x - letter_thickness/2,
                                         intersection_center.y - letter_thickness/2)
        self.v_rect = rg.Rectangle(v_start, rg.Point(v_start.x + letter_thickness, v_start.y + height))
        h_start = rg.Point(intersection_center.x - width/2, intersection_center.y - letter_thickness/2)
        self.h_rect = rg.Rectangle(h_start, rg.Point(h_start.x + width, h_start.y + letter_thickness))
    ...
    def clone(self):
        ...
        #######################################################################
        new_t = CapitalT(self.h_rect.get_center(), self.h_rect.get_width(), self.v_rect.get_height(),
